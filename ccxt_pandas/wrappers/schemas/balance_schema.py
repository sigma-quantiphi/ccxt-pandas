...
class BalanceSchema(BaseExchangeSchema):
    ...
    used: Series[float] = pa.Field(ge=0, title="Used", description="Balance in open orders")
    total: Series[float] = pa.Field(ge=0, title="Total", description="Total balance (free + used)")

    # Optional fields
    debt: Series[float] | None = pa.Field(
